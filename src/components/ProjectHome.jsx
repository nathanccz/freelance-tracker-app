...
export default function ProjectHome({ setActiveRoute }) {
    const { id } = useParams();
    const { projects, handleEditContractAmount, handleEditAmountPaid } = useAppwriteContext()
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4">
                        <div className="card bg-base-100 w-full shadow-xl relative">
                            ...
                            <div className="absolute top-3 right-3 rounded-full p-1 hover:bg-gray-300 duration-300 cursor-pointer">
                                <Icon icon="material-symbols:edit-outline" className='text-3xl' onClick={() => setIsEditingContractAmount(true)}/>    
                            </div>
                            }
                            <div className="card-body text-center">
                                ...
                                <div className="w-full flex flex-col gap-2"> 
                                    ...
                                    <div className="w-full flex justify-end gap-3">
                                        <button className="btn btn-secondary" onClick={() => {setIsEditingContractAmount(false); setContractInput(project?.['contract-amount'])}}>Cancel</button>
                        ...
                        <div className="card bg-base-100 w-full shadow-xl relative">
                            {!isEditingAmountPaid &&
                            <div className="absolute top-3 right-3 rounded-full p-1 hover:bg-gray-300 duration-300 cursor-pointer">
                                <Icon icon="material-symbols:edit-outline" className='text-3xl' onClick={() => setIsEditingAmountPaid(true)}/>
                            </div>}
                            <div className="card-body text-center">
                                <h2 className="card-title mx-auto text-xl"><Icon icon="mdi:leads-outline" className='text-2xl'/>Amount Paid</h2>
                                {isEditingAmountPaid ? 
                                <div className="w-full flex flex-col gap-2"> 
                                    <input type="text" placeholder="Enter new amount" className="input w-full" onChange={(e) => setAmountPaidInput(e.target.value)} value={amountPaidInput}/> 
                                    <div className="w-full flex justify-end gap-3">
                                        <button className="btn btn-secondary" onClick={() => {setIsEditingAmountPaid(false); setAmountPaidInput(project?.['amount-paid'])}}>Cancel</button>
                                        <button className="btn btn-accent" onClick={handleClickSaveAmountPaid}>Save</button>
                                    </div>
                                </div> 
                                : 
                        ...
                        <div className="card bg-base-100 w-full shadow-xl relative">
                            <div className="absolute top-3 right-3 rounded-full p-1 hover:bg-gray-300 duration-300 cursor-pointer">
                                <Icon icon="material-symbols:edit-outline" className='text-3xl' onClick={handleContactsModalOpen}/>
                            </div>
                            <div className="card-body text-center">
